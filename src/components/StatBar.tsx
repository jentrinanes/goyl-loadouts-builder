  ...
  return (
    <div className={compact ? 'mb-1' : 'mb-2'}>
      <div className="flex justify-between mb-0.5">
        <span className={`${compact ? 'text-[11px]' : 'text-xs'} text-gray-600 dark:text-gray-400 uppercase tracking-widest`}>
          {meta.label}
        </span>
        <span className={`${compact ? 'text-[11px]' : 'text-xs'} text-gray-700 dark:text-gray-200 font-semibold`}>
          {value}
        </span>
      </div>
      <div className={`bg-gray-200 dark:bg-gray-800 rounded overflow-hidden ${compact ? 'h-1' : 'h-1.5'}`}>
        <div
