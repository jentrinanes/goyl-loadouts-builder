import type { Gear, GearCategory, GearRarity, GearSlot, MeleeWeaponType, RangedWeaponType, StatKey } from '../types';

export const GEAR_CATEGORIES: Record<string, GearCategory> = {
  MELEE: 'Melee',
  RANGE: 'Range',
  CHARM: 'Charm',
  GHOST_WEAPON: 'Ghost Tool',
};

// Default layout (Mercenary, Shinobi): 3 Melee + 1 Range + 1 Charm + 2 Ghost Tool slots
export const GEAR_SLOTS: GearSlot[] = [
  { id: 'melee1',       label: 'Melee I',       category: 'Melee'      },
  { id: 'melee2',       label: 'Melee II',      category: 'Melee'      },
  { id: 'melee3',       label: 'Melee III',     category: 'Melee'      },
  { id: 'range1',       label: 'Range I',       category: 'Range'      },
  { id: 'charm',        label: 'Charm',         category: 'Charm'      },
  { id: 'ghostWeapon',  label: 'Ghost Tool I',  category: 'Ghost Tool' },
  { id: 'ghostWeapon2', label: 'Ghost Tool II', category: 'Ghost Tool' },
];

// Samurai layout: 4 Melee + 1 Range + 1 Charm + 1 Ghost Tool slot
export const SAMURAI_GEAR_SLOTS: GearSlot[] = [
  { id: 'melee1',      label: 'Melee I',    category: 'Melee'      },
  { id: 'melee2',      label: 'Melee II',   category: 'Melee'      },
  { id: 'melee3',      label: 'Melee III',  category: 'Melee'      },
  { id: 'melee4',      label: 'Melee IV',   category: 'Melee'      },
  { id: 'range1',      label: 'Range I',    category: 'Range'      },
  { id: 'charm',       label: 'Charm',      category: 'Charm'      },
  { id: 'ghostWeapon', label: 'Ghost Tool', category: 'Ghost Tool' },
];

// Archer layout: 3 Melee + 2 Range + 1 Charm + 1 Ghost Tool slot
export const ARCHER_GEAR_SLOTS: GearSlot[] = [
  { id: 'melee1',      label: 'Melee I',    category: 'Melee'      },
  { id: 'melee2',      label: 'Melee II',   category: 'Melee'      },
  { id: 'melee3',      label: 'Melee III',  category: 'Melee'      },
  { id: 'range1',      label: 'Range I',    category: 'Range'      },
  { id: 'range2',      label: 'Range II',   category: 'Range'      },
  { id: 'charm',       label: 'Charm',      category: 'Charm'      },
  { id: 'ghostWeapon', label: 'Ghost Tool', category: 'Ghost Tool' },
];

export const getSlotsForClass = (classId: string | null): GearSlot[] => {
  if (classId === 'archer') return ARCHER_GEAR_SLOTS;
  if (classId === 'samurai') return SAMURAI_GEAR_SLOTS;
  return GEAR_SLOTS;
};

export const ATTR2_OPTIONS: string[] = [
  'Enemy Staggered Damage',
  'Execution Damage',
  'Ultimate Gain',
  'Ultimate Damage',
  'Spirit Move Damage',
  'Thrown Weapon Damage',
  'Perfect Parry Window',
  'Class Ability Cooldown Reduction',
];

const DUAL_KATANA_ATTR1 = ['Dual Katana Damage', 'Dual Katana Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage'];
const DUAL_KATANA_ATTR2 = ['Enemy Staggered Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'];
const DUAL_KATANA_ATTR3 = ['Dual Tempest Strikes', 'Unrelenting Flurry', 'Burning Blade', 'Poison Blade'];

const ODACHI_ATTR1 = ['Odachi Damage', 'Odachi Stagger Damage', 'Odachi Counter Window', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage'];
const ODACHI_ATTR2 = ['Enemy Staggered Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'];
const ODACHI_ATTR3 = ['Expert Combo', 'Unstoppable Force', 'Resolve of the Mountain', 'Burning Blade', 'Poison Blade'];

export const GEARS: Gear[] = [
  // --- MELEE: KATANA ---
  {
    id: 'iaido_katana',
    name: 'Iaido Katana',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🗡️',
    description: 'Effective against Swords',
    stats: { attack: 60, defense: 10, health: 5, resolve: 15, stealth: 10, ranged: 0 },
    perk: 'First strike of each encounter deals bonus damage',
    attributes1: ['Katana Damage', 'Katana Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage'],
    attributes2: ATTR2_OPTIONS,
    attributes3: ['Expert Combo', 'Resolve Of Lightning', 'Burning Blade', 'Poison Blade'],
    attributeMaxValues: {
      'Katana Damage': 11, 'Katana Stagger Damage': 11, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Execution Damage': 27, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'katana',
  },
  {
    id: 'shattering_katana',
    name: 'Shattering Katana',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🗡️',
    description: 'Extremely effective against Swords',
    stats: { attack: 75, defense: 15, health: 10, resolve: 10, stealth: 0, ranged: 0 },
    perk: 'Breaks enemy guard on heavy attacks',
    attributes1: ['Katana Damage', 'Katana Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage'],
    attributes2: ATTR2_OPTIONS,
    attributes3: ['Expert Combo', 'Resolve Of Lightning', 'Burning Blade', 'Poison Blade'],
    attributeMaxValues: {
      'Katana Damage': 11, 'Katana Stagger Damage': 11, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Execution Damage': 27, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'katana',
  },
  {
    id: 'thunderous_katana',
    name: 'Thunderous Katana',
    category: 'Melee',
    rarity: 'Legendary',
    icon: '🗡️',
    description: 'Katana Spirit moves have a 100% chance to throw nearby enemies. Extremely effective against Swords',
    stats: { attack: 90, defense: 15, health: 10, resolve: 20, stealth: 0, ranged: 0 },
    perk: 'Strikes chain lightning to nearby enemies',
    attributes1: ['Katana Damage', 'Katana Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage'],
    attributes2: ATTR2_OPTIONS,
    attributes3: ['Expert Combo', 'Resolve Of Lightning', 'Burning Blade', 'Poison Blade'],
    attributeMaxValues: {
      'Katana Damage': 12, 'Katana Stagger Damage': 12, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 10, 'Perfect Counter Damage': 35,
      'Enemy Staggered Damage': 28, 'Execution Damage': 30, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    weaponType: 'katana',
  },

  // --- MELEE: DUAL KATANA ---
  {
    id: 'relentless_dual_katana',
    name: 'Relentless Dual Katana',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🗡️',
    description: 'Extremely effective against Polearms',
    stats: { attack: 80, defense: 5, health: 5, resolve: 10, stealth: 10, ranged: 0 },
    perk: 'Each consecutive hit increases attack speed',
    attributes1: DUAL_KATANA_ATTR1,
    attributes2: DUAL_KATANA_ATTR2,
    attributes3: DUAL_KATANA_ATTR3,
    attributeMaxValues: {
      'Dual Katana Damage': 11, 'Dual Katana Stagger Damage': 11, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'dual_katana',
  },
  {
    id: 'vigilant_dual_katana',
    name: 'Vigilant Dual Katana',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🗡️',
    description: 'Effective against Polearms. Heavy Attacks automatically parry Polearm attacks',
    stats: { attack: 65, defense: 25, health: 10, resolve: 15, stealth: 5, ranged: 0 },
    perk: 'Parrying with dual katana restores resolve',
    attributes1: DUAL_KATANA_ATTR1,
    attributes2: DUAL_KATANA_ATTR2,
    attributes3: DUAL_KATANA_ATTR3,
    attributeMaxValues: {
      'Dual Katana Damage': 11, 'Dual Katana Stagger Damage': 11, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'dual_katana',
  },
  {
    id: 'tempest_dual_katana',
    name: 'Tempest Dual Katana',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🗡️',
    description: 'Effective against Polearms',
    stats: { attack: 70, defense: 10, health: 5, resolve: 15, stealth: 10, ranged: 0 },
    perk: 'Tempest strikes build momentum with each hit',
    attributes1: DUAL_KATANA_ATTR1,
    attributes2: DUAL_KATANA_ATTR2,
    attributes3: DUAL_KATANA_ATTR3,
    attributeMaxValues: {
      'Dual Katana Damage': 11, 'Dual Katana Stagger Damage': 11, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'dual_katana',
  },
  {
    id: 'lightnings_bite',
    name: "Lightning's Bite",
    category: 'Melee',
    rarity: 'Legendary',
    icon: '⚡',
    description: "Takezo's Dual Slash calls down a lightning strike at the end of the combo. Effective against Polearms",
    stats: { attack: 85, defense: 10, health: 5, resolve: 25, stealth: 5, ranged: 0 },
    perk: 'Dual strikes have a chance to stun enemies',
    attributes1: DUAL_KATANA_ATTR1,
    attributes2: DUAL_KATANA_ATTR2,
    attributes3: DUAL_KATANA_ATTR3,
    attributeMaxValues: {
      'Dual Katana Damage': 12, 'Dual Katana Stagger Damage': 12, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 10, 'Perfect Counter Damage': 35,
      'Enemy Staggered Damage': 28, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    weaponType: 'dual_katana',
  },

  // --- MELEE: YARI ---
  {
    id: 'relentless_yari',
    name: 'Relentless Yari',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🔱',
    description: 'Extremely effective against Sickles',
    stats: { attack: 70, defense: 10, health: 10, resolve: 5, stealth: 0, ranged: 0 },
    perk: 'Spear thrusts have extended reach',
    attributes1: ['Yari Damage', 'Yari Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage'],
    attributes2: ['Enemy Staggered Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributes3: ['Expert Combo', 'Quickening Tides', 'Strength Of Typhoons', 'Burning Blade', 'Poison Blade'],
    attributeMaxValues: {
      'Yari Damage': 11, 'Yari Stagger Damage': 11, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'yari',
  },
  {
    id: 'swift_yari',
    name: 'Swift Yari',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🔱',
    description: 'Effective against Sickles',
    stats: { attack: 60, defense: 5, health: 5, resolve: 10, stealth: 15, ranged: 0 },
    perk: 'Faster recovery after each thrust',
    attributes1: ['Yari Damage', 'Yari Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage'],
    attributes2: ['Enemy Staggered Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributes3: ['Expert Combo', 'Quickening Tides', 'Strength Of Typhoons', 'Burning Blade', 'Poison Blade'],
    attributeMaxValues: {
      'Yari Damage': 11, 'Yari Stagger Damage': 11, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'yari',
  },
  {
    id: 'vigilant_yari',
    name: 'Vigilant Yari',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🔱',
    description: 'Effective against Sickles. Heavy Attacks automatically parry Sickles',
    stats: { attack: 65, defense: 30, health: 15, resolve: 10, stealth: 0, ranged: 0 },
    perk: 'Counter-attacks deal significantly more damage',
    attributes1: ['Yari Damage', 'Yari Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage'],
    attributes2: ['Enemy Staggered Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributes3: ['Expert Combo', 'Quickening Tides', 'Strength Of Typhoons', 'Burning Blade', 'Poison Blade'],
    attributeMaxValues: {
      'Yari Damage': 11, 'Yari Stagger Damage': 11, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'yari',
  },
  {
    id: 'thunderous_yari',
    name: 'Thunderous Yari',
    category: 'Melee',
    rarity: 'Legendary',
    icon: '⚡',
    description: 'Thundering Wave creates a blast that Weakens nearby enemies',
    stats: { attack: 88, defense: 20, health: 10, resolve: 20, stealth: 0, ranged: 0 },
    perk: 'Charged thrusts release a lightning shockwave',
    attributes1: ['Yari Damage', 'Yari Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage'],
    attributes2: ['Enemy Staggered Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributes3: ['Expert Combo', 'Quickening Tides', 'Strength Of Typhoons', 'Burning Blade', 'Poison Blade'],
    attributeMaxValues: {
      'Yari Damage': 12, 'Yari Stagger Damage': 12, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 10, 'Perfect Counter Damage': 35,
      'Enemy Staggered Damage': 28, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    weaponType: 'yari',
  },

  // --- MELEE: KUSARIGAMA ---
  {
    id: 'nimble_kusarigama',
    name: 'Nimble Kusarigama',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🌀',
    description: 'Effective against Shields',
    stats: { attack: 60, defense: 10, health: 5, resolve: 15, stealth: 20, ranged: 0 },
    perk: 'Dodge distance increased while equipped',
    attributes1: ['Kusarigama Damage', 'Kusarigama Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage', 'Assassination Spirit Gain'],
    attributes2: ['Assassination Damage', 'Enemy Staggered Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributes3: ['Expert Combo', 'Shattering Strike', 'Burning Blade', 'Poison Blade'],
    attributeMaxValues: {
      'Kusarigama Damage': 11, 'Kusarigama Stagger Damage': 11, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30, 'Assassination Spirit Gain': 11,
      'Assassination Damage': 21, 'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'kusarigama',
  },
  {
    id: 'relentless_kusarigama',
    name: 'Relentless Kusarigama',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🌀',
    description: 'Extremely effective against Shields',
    stats: { attack: 78, defense: 5, health: 5, resolve: 10, stealth: 10, ranged: 0 },
    perk: 'Chain hits reduce enemy block effectiveness',
    attributes1: ['Kusarigama Damage', 'Kusarigama Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage', 'Assassination Spirit Gain'],
    attributes2: ['Assassination Damage', 'Enemy Staggered Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributes3: ['Expert Combo', 'Shattering Strike', 'Burning Blade', 'Poison Blade'],
    attributeMaxValues: {
      'Kusarigama Damage': 11, 'Kusarigama Stagger Damage': 11, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30, 'Assassination Spirit Gain': 11,
      'Assassination Damage': 21, 'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'kusarigama',
  },
  {
    id: 'vanishing_kusarigama',
    name: 'Vanishing Kusarigama',
    category: 'Melee',
    rarity: 'Legendary',
    icon: '🌀',
    description: 'Killing enemies with Claw and Talon makes you Vanish. Extremely effective against Shields',
    stats: { attack: 75, defense: 10, health: 5, resolve: 20, stealth: 40, ranged: 0 },
    perk: 'Kills with this weapon briefly turn you invisible',
    attributes1: ['Kusarigama Damage', 'Kusarigama Stagger Damage', 'All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage', 'Assassination Spirit Gain'],
    attributes2: ['Assassination Damage', 'Enemy Staggered Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributes3: ['Expert Combo', 'Shattering Strike', 'Burning Blade', 'Poison Blade'],
    attributeMaxValues: {
      'Kusarigama Damage': 12, 'Kusarigama Stagger Damage': 12, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 10, 'Perfect Counter Damage': 35, 'Assassination Spirit Gain': 12,
      'Assassination Damage': 23, 'Enemy Staggered Damage': 28, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    weaponType: 'kusarigama',
  },

  // --- MELEE: ODACHI ---
  {
    id: 'relentless_odachi',
    name: 'Relentless Odachi',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🏯',
    description: 'Extremely effective against Brutes',
    stats: { attack: 90, defense: 15, health: 10, resolve: 5, stealth: 0, ranged: 0 },
    perk: 'Relentless strikes against heavy enemies deal bonus damage',
    attributes1: ODACHI_ATTR1,
    attributes2: ODACHI_ATTR2,
    attributes3: ODACHI_ATTR3,
    attributeMaxValues: {
      'Odachi Damage': 11, 'Odachi Stagger Damage': 11, 'Odachi Counter Window': 16.6, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'odachi',
  },
  {
    id: 'vigilant_odachi',
    name: 'Vigilant Odachi',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🏯',
    description: 'Effective against Brutes',
    stats: { attack: 75, defense: 30, health: 15, resolve: 10, stealth: 0, ranged: 0 },
    perk: 'Counter-attacks against heavy enemies deal bonus damage',
    attributes1: ODACHI_ATTR1,
    attributes2: ODACHI_ATTR2,
    attributes3: ODACHI_ATTR3,
    attributeMaxValues: {
      'Odachi Damage': 11, 'Odachi Stagger Damage': 11, 'Odachi Counter Window': 16.6, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'odachi',
  },
  {
    id: 'restoring_odachi',
    name: 'Restoring Odachi',
    category: 'Melee',
    rarity: 'Epic',
    icon: '🏯',
    description: 'Effective against Brutes. Heal when using Execution Strikes on knocked down enemies',
    stats: { attack: 75, defense: 20, health: 35, resolve: 15, stealth: 0, ranged: 0 },
    perk: 'Kills restore a portion of health',
    attributes1: ODACHI_ATTR1,
    attributes2: ODACHI_ATTR2,
    attributes3: ODACHI_ATTR3,
    attributeMaxValues: {
      'Odachi Damage': 11, 'Odachi Stagger Damage': 11, 'Odachi Counter Window': 16.6, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Enemy Staggered Damage': 24, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    weaponType: 'odachi',
  },
  {
    id: 'blade_of_mountains',
    name: 'Blade of Mountains',
    category: 'Melee',
    rarity: 'Legendary',
    icon: '🗻',
    description: 'Mountain Breaker heals on impact. Extremely effective against Brutes',
    stats: { attack: 95, defense: 35, health: 20, resolve: 10, stealth: 0, ranged: 0 },
    perk: 'Cannot be staggered while attacking',
    attributes1: ODACHI_ATTR1,
    attributes2: ODACHI_ATTR2,
    attributes3: ODACHI_ATTR3,
    attributeMaxValues: {
      'Odachi Damage': 12, 'Odachi Stagger Damage': 12, 'Odachi Counter Window': 20.7, 'All Melee Damage': 5, 'All Melee Stagger Damage': 5,
      'Spirit Gain On Melee': 10, 'Perfect Counter Damage': 35,
      'Enemy Staggered Damage': 28, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    weaponType: 'odachi',
  },

  // --- RANGE: HANKYU (Short Bow) ---
  {
    id: 'hankyu',
    name: 'Hankyu',
    category: 'Range',
    rarity: 'Epic',
    icon: '🏹',
    description: 'Average damage and slow draw time',
    stats: { attack: 45, defense: 5, health: 5, resolve: 10, stealth: 10, ranged: 60 },
    perk: 'Faster draw speed on consecutive shots',
    attributes1: ['Hankyu Damage', 'Hankyu Speed', 'Hankyu Stability', 'All Ranged Speed', 'All Ranged Headshot Damage', 'Ranged Spirit Gain'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Silent Arrow', 'Critical Release', 'Iron Grip', 'Fire Arrows', 'Poison Arrows'],
    attributeMaxValues: {
      'Hankyu Damage': 11, 'Hankyu Speed': 11, 'Hankyu Stability': 35, 'All Ranged Speed': 11, 'All Ranged Headshot Damage': 11, 'Ranged Spirit Gain': 9,
      'All Ranged Damage': 22, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36,
    },
    rangedWeaponType: 'hankyu' as const,
  },
  {
    id: 'storm_hankyu',
    name: 'Storm Hankyu',
    category: 'Range',
    rarity: 'Epic',
    icon: '🏹',
    description: 'A Hankyu capable of rapid fire shots, but cannot be aimed',
    stats: { attack: 55, defense: 5, health: 0, resolve: 10, stealth: 5, ranged: 70 },
    perk: 'Rapid shots build up a static charge',
    attributes1: ['Hankyu Damage', 'Hankyu Speed', 'Hankyu Stability', 'All Ranged Speed', 'All Ranged Headshot Damage', 'Ranged Spirit Gain'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Fire Arrows', 'Poison Arrows'],
    attributeMaxValues: {
      'Hankyu Damage': 11, 'Hankyu Speed': 11, 'Hankyu Stability': 35, 'All Ranged Speed': 11, 'All Ranged Headshot Damage': 11, 'Ranged Spirit Gain': 9,
      'All Ranged Damage': 22, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36,
    },
    rangedWeaponType: 'hankyu' as const,
  },
  {
    id: 'hurricane_hankyu',
    name: 'Hurricane Hankyu',
    category: 'Range',
    rarity: 'Legendary',
    icon: '🌪️',
    description: 'Fires at double speed, but cannot be aimed. Its arrows inflict Maim, making enemies briefly Vulnerable to any Melee weapon',
    stats: { attack: 65, defense: 5, health: 5, resolve: 20, stealth: 10, ranged: 85 },
    perk: 'Volleys of arrows knock back groups of enemies',
    attributes1: ['Hankyu Damage', 'Hankyu Speed', 'Hankyu Stability', 'All Ranged Speed', 'All Ranged Headshot Damage', 'Ranged Spirit Gain'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Fire Arrows', 'Poison Arrows'],
    attributeMaxValues: {
      'Hankyu Damage': 12, 'Hankyu Speed': 12, 'Hankyu Stability': 35, 'All Ranged Speed': 12, 'All Ranged Headshot Damage': 12, 'Ranged Spirit Gain': 10,
      'All Ranged Damage': 25, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
    rangedWeaponType: 'hankyu' as const,
  },

  {
    id: 'weightless_spirit',
    name: 'Weightless Spirit',
    category: 'Range',
    rarity: 'Legendary',
    icon: '🏹',
    description: 'Fast firing and deadly accurate. Arrows shot by this bow pierce enemy shields and helmets',
    stats: { attack: 0, defense: 0, health: 0, resolve: 0, stealth: 0, ranged: 0 },
    perk: '',
    attributes1: ['Hankyu Damage', 'Hankyu Speed', 'Hankyu Stability', 'All Ranged Speed', 'All Ranged Headshot Damage', 'Ranged Spirit Gain'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Silent Arrows', 'Critical Release', 'Iron Grip', 'Fire Arrows', 'Poison Arrows'],
    attributeMaxValues: {
      'Hankyu Damage': 12, 'Hankyu Speed': 12, 'Hankyu Stability': 35, 'All Ranged Speed': 12, 'All Ranged Headshot Damage': 12, 'Ranged Spirit Gain': 10,
      'All Ranged Damage': 25, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
    rangedWeaponType: 'hankyu' as const,
  },

  // --- RANGE: TANEGASHIMA (Rifle) ---
  {
    id: 'tanegashima',
    name: 'Tanegashima',
    category: 'Range',
    rarity: 'Epic',
    icon: '🔫',
    description: 'Extremely high damage, but a very long reload time',
    stats: { attack: 80, defense: 5, health: 0, resolve: 5, stealth: 0, ranged: 80 },
    perk: 'Fully-charged shot penetrates armor',
    attributes1: ['Tanegashima Damage', 'Tanegashima Reload Speed', 'All Ranged Headshot Damage', 'Ranged Spirit Gain'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Extra Ammo', 'Shrapnel Bullets', 'Auto Reload'],
    attributeMaxValues: {
      'Tanegashima Damage': 11, 'Tanegashima Reload Speed': 12, 'All Ranged Headshot Damage': 11, 'Ranged Spirit Gain': 9,
      'All Ranged Damage': 22, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36,
    },
    rangedWeaponType: 'tanegashima' as const,
  },
  {
    id: 'lightning_tanegashima',
    name: 'Lightning Tanegashima',
    category: 'Range',
    rarity: 'Legendary',
    icon: '⚡',
    description: 'Reloads instantly with a headshot or killing an enemy with the Tanegashima',
    stats: { attack: 95, defense: 5, health: 0, resolve: 15, stealth: 0, ranged: 95 },
    perk: 'Shots release a lightning burst on impact',
    attributes1: ['Tanegashima Damage', 'Tanegashima Reload Speed', 'All Ranged Headshot Damage', 'Ranged Spirit Gain'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Extra Ammo', 'Shrapnel Bullets', 'Auto Reload'],
    attributeMaxValues: {
      'Tanegashima Damage': 12, 'Tanegashima Reload Speed': 14, 'All Ranged Headshot Damage': 12, 'Ranged Spirit Gain': 10,
      'All Ranged Damage': 25, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
    rangedWeaponType: 'tanegashima' as const,
  },

  // --- RANGE: YUMI (Long Bow) ---
  {
    id: 'yumi',
    name: 'Yumi',
    category: 'Range',
    rarity: 'Epic',
    icon: '🏹',
    description: 'High damage and slow draw time',
    stats: { attack: 60, defense: 5, health: 10, resolve: 10, stealth: 5, ranged: 70 },
    perk: 'Drawn shots deal bonus damage beyond close range',
    attributes1: ['Yumi Damage', 'Yumi Speed', 'Yumi Stability', 'All Ranged Speed', 'All Ranged Headshot Damage', 'Ranged Spirit Gain'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributes3: ['Silent Arrow', 'Critical Release', 'Iron Grip', 'Disarm Arrows'],
    attributeMaxValues: {
      'Yumi Damage': 11, 'Yumi Speed': 11, 'Yumi Stability': 35, 'All Ranged Speed': 11, 'All Ranged Headshot Damage': 11, 'Ranged Spirit Gain': 9,
      'All Ranged Damage': 22, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11
    },
    rangedWeaponType: 'yumi' as const,
  },
  {
    id: 'skipping_stone_bow',
    name: 'Skipping Stone Bow',
    category: 'Range',
    rarity: 'Legendary',
    icon: '🏹',
    description: 'Headshots ricochet to a nearby enemy',
    stats: { attack: 70, defense: 0, health: 5, resolve: 20, stealth: 15, ranged: 85 },
    perk: 'Arrows ricochet to hit up to 2 additional enemies',
    attributes1: ['Yumi Damage', 'Yumi Speed', 'Yumi Stability', 'All Ranged Speed', 'All Ranged Headshot Damage', 'Ranged Spirit Gain'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributes3: ['Silent Arrow', 'Critical Release', 'Iron Grip', 'Disarm Arrows'],
    attributeMaxValues: {
      'Yumi Damage': 12, 'Yumi Speed': 12, 'Yumi Stability': 35, 'All Ranged Speed': 12, 'All Ranged Headshot Damage': 12, 'Ranged Spirit Gain': 10,
      'All Ranged Damage': 25, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12
    },
    rangedWeaponType: 'yumi' as const,
  },
  {
    id: 'true_aim_yumi',
    name: 'True Aim Yumi',
    category: 'Range',
    rarity: 'Legendary',
    icon: '🏹',
    description: 'Locks-on to enemies and fires 3 arrows at once, inflicting massive damage',
    stats: { attack: 65, defense: 5, health: 5, resolve: 10, stealth: 5, ranged: 90 },
    perk: 'Aim sway is eliminated at full draw',
    attributes1: ['Yumi Damage', 'Yumi Speed', 'Yumi Stability', 'All Ranged Speed', 'All Ranged Headshot Damage', 'Ranged Spirit Gain'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributes3: ['Silent Arrow', 'Critical Release', 'Iron Grip', 'Disarm Arrows'],
    attributeMaxValues: {
      'Yumi Damage': 12, 'Yumi Speed': 12, 'Yumi Stability': 35, 'All Ranged Speed': 12, 'All Ranged Headshot Damage': 12, 'Ranged Spirit Gain': 10,
      'All Ranged Damage': 25, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12
    },
    rangedWeaponType: 'yumi' as const,
  },

  // --- RANGE: BOMBS ---
  {
    id: 'concussion_bomb',
    name: 'Concussion Bomb',
    category: 'Range',
    rarity: 'Epic',
    icon: '💣',
    description: 'Briefly interrupts enemies caught in its damaging blast',
    stats: { attack: 55, defense: 0, health: 0, resolve: 5, stealth: 0, ranged: 40 },
    perk: 'Staggered enemies take increased damage',
    attributes1: ['All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage', 'Ranged Spirit Gain', 'Concussion Bomb Radius'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Spirited', 'Scorch Bombs'],
    attributeMaxValues: {
      'All Melee Damage': 5, 'All Melee Stagger Damage': 5, 'Spirit Gain On Melee': 9, 'Perfect Counter Damage': 30,
      'Ranged Spirit Gain': 9, 'Concussion Bomb Radius': 250,
      'All Ranged Damage': 22, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36,
    },
    rangedWeaponType: 'bomb' as const,
  },
  {
    id: 'vengeful_onibi_bomb',
    name: 'Vengeful Onibi Bomb',
    category: 'Range',
    rarity: 'Legendary',
    icon: '🔥',
    description: 'Creates friendly Onibi within its concussion bomb blast',
    stats: { attack: 75, defense: 0, health: 0, resolve: 15, stealth: 0, ranged: 50 },
    perk: 'Spirit fire lingers and spreads between enemies',
    attributes1: ['All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage', 'Ranged Spirit Gain', 'Concussion Bomb Radius'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Spirited', 'Scorch Bombs'],
    attributeMaxValues: {
      'All Melee Damage': 5, 'All Melee Stagger Damage': 5, 'Spirit Gain On Melee': 10, 'Perfect Counter Damage': 35,
      'Ranged Spirit Gain': 10, 'Concussion Bomb Radius': 300,
      'All Ranged Damage': 25, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
    rangedWeaponType: 'bomb' as const,
  },
  {
    id: 'blind_bomb',
    name: 'Blind Bomb',
    category: 'Range',
    rarity: 'Epic',
    icon: '💨',
    description: 'Blinds enemies within a small radius and creates a Shrouded Area that conceals your movements',
    stats: { attack: 20, defense: 15, health: 0, resolve: 10, stealth: 30, ranged: 30 },
    perk: 'Blinded enemies lose track of your position',
    attributes1: ['Ranged Spirit Gain', 'Assassination Damage', 'Assassination Spirit Gain', 'Blind Bomb Radius', 'Blind Bomb Duration'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Spirited', 'Increased Impact'],
    attributeMaxValues: {
      'Ranged Spirit Gain': 9, 'Assassination Damage': 9, 'Assassination Spirit Gain': 11,
      'Blind Bomb Radius': 11, 'Blind Bomb Duration': 5,
      'All Ranged Damage': 22, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36,
    },
    rangedWeaponType: 'bomb' as const,
  },
  {
    id: 'healing_blind_bomb',
    name: 'Healing Blind Bomb',
    category: 'Range',
    rarity: 'Legendary',
    icon: '✨',
    description: 'Heals allies and Blinds enemies with a Shrouded Area that conceals your movements',
    stats: { attack: 20, defense: 10, health: 30, resolve: 20, stealth: 20, ranged: 30 },
    perk: 'Restores health to self and allies caught in the blast',
    attributes1: ['Ranged Spirit Gain', 'Assassination Damage', 'Assassination Spirit Gain', 'Blind Bomb Radius', 'Blind Bomb Duration'],
    attributes2: ['All Ranged Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Spirited', 'Increased Impact'],
    attributeMaxValues: {
      'Ranged Spirit Gain': 10, 'Assassination Damage': 10, 'Assassination Spirit Gain': 12,
      'Blind Bomb Radius': 14, 'Blind Bomb Duration': 6,
      'All Ranged Damage': 25, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
    rangedWeaponType: 'bomb' as const,
  },

  // --- CHARM ---
  {
    id: 'basic_charm',
    name: 'Basic Charm',
    category: 'Charm',
    rarity: 'Epic',
    icon: '🔮',
    description: 'Basic defensive charm with no special abilities',
    stats: { attack: 5, defense: 15, health: 15, resolve: 10, stealth: 5, ranged: 5 },
    perk: 'Minor boost to all defensive stats',
    attributes1: ['Spirit Gain', 'Spirit Heal Effect', 'Weapon Aligned Stagger Self-Heal', 'Incoming Damage Reduction'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Spirit Gain': 22, 'Spirit Heal Effect': 45, 'Weapon Aligned Stagger Self-Heal': 11, 'Incoming Damage Reduction': 9,
      'Assassination Damage': 21, 'Status Effect Duration': 22, 'Status Effect Damage': 22, 'Enemy Staggered Damage': 24,
      'Execution Damage': 27, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'senseis_guidance',
    name: "Sensei's Guidance",
    category: 'Charm',
    rarity: 'Epic',
    icon: '🔮',
    description: 'A charm to train others. Sacrifice all your mission earned XP and distribute to other players',
    stats: { attack: 0, defense: 0, health: 0, resolve: 0, stealth: 0, ranged: 0 },
    perk: '',
    attributes1: ['Spirit Gain', 'Spirit Heal Effect', 'Weapon Aligned Stagger Self-Heal', 'Incoming Damage Reduction'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Spirit Gain': 22, 'Spirit Heal Effect': 45, 'Weapon Aligned Stagger Self-Heal': 11, 'Incoming Damage Reduction': 9,
      'Assassination Damage': 21, 'Status Effect Duration': 22, 'Status Effect Damage': 22, 'Enemy Staggered Damage': 24,
      'Execution Damage': 27, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'samurais_honor',
    name: "Samurai's Honor",
    category: 'Charm',
    rarity: 'Epic',
    icon: '🔮',
    description: 'A charm with Melee-focused properties',
    stats: { attack: 0, defense: 0, health: 0, resolve: 0, stealth: 0, ranged: 0 },
    perk: '',
    attributes1: ['All Melee Damage', 'All Melee Stagger Damage', 'Enemy Staggered Damage', 'Perfect Counter Damage'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'All Melee Damage': 15, 'All Melee Stagger Damage': 15, 'Enemy Staggered Damage': 30, 'Perfect Counter Damage': 35,
      'Assassination Damage': 21, 'Status Effect Duration': 22, 'Status Effect Damage': 22,
      'Execution Damage': 27, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'daimyos_generosity',
    name: "Daimyo's Generosity",
    category: 'Charm',
    rarity: 'Epic',
    icon: '🔮',
    description: 'A charm to enrich others. Sacrifice all your mission reward currency and distribute it to other players',
    stats: { attack: 0, defense: 0, health: 0, resolve: 0, stealth: 0, ranged: 0 },
    perk: '',
    attributes1: ['Spirit Gain', 'Spirit Heal Effect', 'Weapon Aligned Stagger Self-Heal', 'Incoming Damage Reduction'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Spirit Gain': 22, 'Spirit Heal Effect': 45, 'Weapon Aligned Stagger Self-Heal': 11, 'Incoming Damage Reduction': 9,
      'Assassination Damage': 21, 'Status Effect Duration': 22, 'Status Effect Damage': 22, 'Enemy Staggered Damage': 24,
      'Execution Damage': 27, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'shadows_bite',
    name: "Shadow's Bite",
    category: 'Charm',
    rarity: 'Epic',
    icon: '🔮',
    description: 'A charm with stealth-focused properties',
    stats: { attack: 0, defense: 0, health: 0, resolve: 0, stealth: 0, ranged: 0 },
    perk: '',
    attributes1: ['Assassination Damage', 'Assassination Spirit Gain', 'Status Effect Duration', 'Status Effect Damage'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Assassination Damage': 23, 'Assassination Spirit Gain': 30, 'Status Effect Duration': 25, 'Status Effect Damage': 25,
      'Enemy Staggered Damage': 24, 'Execution Damage': 27, 'Ultimate Gain': 12, 'Ultimate Damage': 21,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'spirit_brew',
    name: 'Spirit Brew',
    category: 'Charm',
    rarity: 'Legendary',
    icon: '🍶',
    description: 'Hold the down arrow button to consume Sake. Grants 2 Spirit, but drinking too much makes you drunk',
    stats: { attack: 0, defense: 0, health: 15, resolve: 50, stealth: 5, ranged: 0 },
    perk: 'Resolve regenerates 25% faster out of combat',
    attributes1: ['Spirit Gain', 'Spirit Heal Effect', 'Weapon Aligned Stagger Self-Heal', 'Incoming Damage Reduction'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Spirit Gain': 25, 'Spirit Heal Effect': 50, 'Weapon Aligned Stagger Self-Heal': 12, 'Incoming Damage Reduction': 10,
      'Assassination Damage': 23, 'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Enemy Staggered Damage': 28,
      'Execution Damage': 30, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'harmonious_bell',
    name: 'Harmonious Bell',
    category: 'Charm',
    rarity: 'Legendary',
    icon: '🔔',
    description: 'Using Healing Bells heals and revives all players',
    stats: { attack: 10, defense: 10, health: 20, resolve: 25, stealth: 10, ranged: 10 },
    perk: 'Balanced stat gains after each kill',
    attributes1: ['Spirit Gain', 'Spirit Heal Effect', 'Weapon Aligned Stagger Self-Heal', 'Incoming Damage Reduction'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Spirit Gain': 25, 'Spirit Heal Effect': 50, 'Weapon Aligned Stagger Self-Heal': 12, 'Incoming Damage Reduction': 10,
      'Assassination Damage': 23, 'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Enemy Staggered Damage': 28,
      'Execution Damage': 30, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'risky_parry',
    name: 'Risky Parry',
    category: 'Charm',
    rarity: 'Legendary',
    icon: '⚡',
    description: 'Regular parries are disabled. Perfect Parries enable 3 counterattacks',
    stats: { attack: 30, defense: 20, health: 0, resolve: 20, stealth: 0, ranged: 0 },
    perk: 'Perfect parries deal 50% bonus counter damage',
    attributes1: ['Spirit Gain', 'Spirit Heal Effect', 'Weapon Aligned Stagger Self-Heal', 'Incoming Damage Reduction'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Spirit Gain': 25, 'Spirit Heal Effect': 50, 'Weapon Aligned Stagger Self-Heal': 12, 'Incoming Damage Reduction': 10,
      'Assassination Damage': 23, 'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Enemy Staggered Damage': 28,
      'Execution Damage': 30, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'archers_supplies',
    name: "Archer's Supplies",
    category: 'Charm',
    rarity: 'Legendary',
    icon: '🎯',
    description: 'Refill 15% of all ammo types. Stacks with Resupply perk',
    stats: { attack: 10, defense: 0, health: 10, resolve: 10, stealth: 15, ranged: 40 },
    perk: 'Ranged ammo capacity increased; headshots restore resolve',
    attributes1: ['Spirit Gain', 'Spirit Heal Effect', 'Weapon Aligned Stagger Self-Heal', 'Incoming Damage Reduction'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Spirit Gain': 25, 'Spirit Heal Effect': 50, 'Weapon Aligned Stagger Self-Heal': 12, 'Incoming Damage Reduction': 10,
      'Assassination Damage': 23, 'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Enemy Staggered Damage': 28,
      'Execution Damage': 30, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'samurais_bracers',
    name: "Samurai's Bracers",
    category: 'Charm',
    rarity: 'Legendary',
    icon: '🛡️',
    description: 'Deal a major amount of Stagger damage to enemies when Parrying their attack',
    stats: { attack: 25, defense: 30, health: 10, resolve: 20, stealth: 0, ranged: 0 },
    perk: 'Stance attacks deal bonus damage when resolve is full',
    attributes1: ['Spirit Gain', 'Spirit Heal Effect', 'Weapon Aligned Stagger Self-Heal', 'Incoming Damage Reduction'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Spirit Gain': 25, 'Spirit Heal Effect': 50, 'Weapon Aligned Stagger Self-Heal': 12, 'Incoming Damage Reduction': 10,
      'Assassination Damage': 23, 'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Enemy Staggered Damage': 28,
      'Execution Damage': 30, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'mercenarys_best_friend',
    name: "Mercenary's Best Friend",
    category: 'Charm',
    rarity: 'Legendary',
    icon: '💰',
    description: 'Spirit Animal Class Ability will summon a Bear',
    stats: { attack: 15, defense: 25, health: 40, resolve: 5, stealth: 0, ranged: 0 },
    perk: 'Survive one lethal hit per encounter with bonus health regen',
    attributes1: ['Spirit Gain', 'Spirit Heal Effect', 'Weapon Aligned Stagger Self-Heal', 'Incoming Damage Reduction'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Spirit Gain': 25, 'Spirit Heal Effect': 50, 'Weapon Aligned Stagger Self-Heal': 12, 'Incoming Damage Reduction': 10,
      'Assassination Damage': 23, 'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Enemy Staggered Damage': 28,
      'Execution Damage': 30, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
  },
  {
    id: 'shinobis_shadow',
    name: "Shinobi's Shadow",
    category: 'Charm',
    rarity: 'Legendary',
    icon: '🌑',
    description: 'Successfully Assassinating an enemy while Class Ability is active re-activates Vanish and refreshes the duration',
    stats: { attack: 10, defense: 0, health: 5, resolve: 15, stealth: 50, ranged: 0 },
    perk: 'Stealth re-engages 40% faster after breaking cover',
    attributes1: ['Spirit Gain', 'Spirit Heal Effect', 'Weapon Aligned Stagger Self-Heal', 'Incoming Damage Reduction'],
    attributes2: ['Assassination Damage', 'Status Effect Duration', 'Status Effect Damage', 'Enemy Staggered Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window'],
    attributes3: ['Bullet Defense', 'Spirited', 'Parry Heal', 'Fire Master', 'Healthy', 'Scavenger', 'Foul Arrows'],
    attributeMaxValues: {
      'Spirit Gain': 25, 'Spirit Heal Effect': 50, 'Weapon Aligned Stagger Self-Heal': 12, 'Incoming Damage Reduction': 10,
      'Assassination Damage': 23, 'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Enemy Staggered Damage': 28,
      'Execution Damage': 30, 'Ultimate Gain': 16, 'Ultimate Damage': 25,
      'Spirit Move Damage': 20, 'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36,
    },
  },

  // --- GHOST TOOL ---
  {
    id: 'kunai',
    name: 'Kunai',
    category: 'Ghost Tool',
    rarity: 'Epic',
    icon: '🗡️',
    description: 'Throwable kunai',
    stats: { attack: 40, defense: 0, health: 0, resolve: 5, stealth: 20, ranged: 40 },
    perk: 'Kunai stagger enemies briefly on hit',
    attributes1: ['Kunai Damage', 'Assassination Damage', 'Assassination Spirit Gain', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'Kunai Damage': 11, 'Assassination Damage': 9, 'Assassination Spirit Gain': 11, 
      'Ghost Tools Spirit Gain': 32, 'Ghost Tools Cooldown Reduction': 11,
      'Status Effect Duration': 22, 'Status Effect Damage': 22, 'Execution Damage': 27,
      'Ultimate Gain': 12, 'Ultimate Damage': 21, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    attributes3: ['Fire Kunai', 'Hidden Blades'],
  },
  {
    id: 'spirit_kunai',
    name: 'Spirit Kunai',
    category: 'Ghost Tool',
    rarity: 'Legendary',
    icon: '✨',
    description: 'Lowers all cooldowns by 10 seconds when used to kill an enemy',
    stats: { attack: 55, defense: 0, health: 0, resolve: 20, stealth: 25, ranged: 50 },
    perk: 'Hits restore resolve and apply spirit burn',
    attributes1: ['Kunai Damage', 'Assassination Damage', 'Assassination Spirit Gain', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'Kunai Damage': 12, 'Assassination Damage': 10, 'Assassination Spirit Gain': 12, 
      'Ghost Tools Spirit Gain': 35, 'Ghost Tools Cooldown Reduction': 12,
      'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Execution Damage': 30,
      'Ultimate Gain': 16, 'Ultimate Damage': 25, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    attributes3: ['Fire Kunai', 'Hidden Blades'],
  },
  {
    id: 'metsubushi',
    name: 'Metsubushi',
    category: 'Ghost Tool',
    rarity: 'Epic',
    icon: '💨',
    description: 'Blinds nearby enemies with a quick cloud of powder',
    stats: { attack: 10, defense: 10, health: 0, resolve: 10, stealth: 45, ranged: 20 },
    perk: 'Blinded enemies cannot raise the alarm',
    attributes1: ['All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'All Melee Damage': 5, 'All Melee Stagger Damage': 5, 'Spirit Gain On Melee': 9,
      'Perfect Counter Damage': 30, 'Ghost Tools Spirit Gain': 32, 'Ghost Tools Cooldown Reduction': 11,
      'Status Effect Duration': 22, 'Status Effect Damage': 22, 'Execution Damage': 27,
      'Ultimate Gain': 12, 'Ultimate Damage': 21, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    attributes3: ['Poison', 'Pestilence'],
  },
  {
    id: 'hallucinating_metsubushi',
    name: 'Hallucinating Metsubushi',
    category: 'Ghost Tool',
    rarity: 'Legendary',
    icon: '🌀',
    description: 'Inflicts Hallucination which causes most enemies to fight for you for a short time',
    stats: { attack: 20, defense: 10, health: 0, resolve: 15, stealth: 40, ranged: 25 },
    perk: 'Affected enemies attack their allies for 8 seconds',
    attributes1: ['All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'All Melee Damage': 5, 'All Melee Stagger Damage': 5, 'Spirit Gain On Melee': 10,
      'Perfect Counter Damage': 35, 'Ghost Tools Spirit Gain': 35, 'Ghost Tools Cooldown Reduction': 12,
      'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Execution Damage': 30,
      'Ultimate Gain': 16, 'Ultimate Damage': 25, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    attributes3: ['Poison', 'Pestilence'],
  },
  {
    id: 'tanzutsu',
    name: 'Tanzutsu',
    category: 'Ghost Tool',
    rarity: 'Epic',
    icon: '🔫',
    description: 'Can be quickly fired to interrupt and stun an enemy',
    stats: { attack: 65, defense: 0, health: 0, resolve: 5, stealth: 0, ranged: 50 },
    perk: 'Close-range shots knock enemies off their feet',
    attributes1: ['Tanzutsu Damage', 'All Ranged Damage', 'Ranged Spirit Gain', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'Tanzutsu Damage': 11, 'All Ranged Damage': 5, 'Ranged Spirit Gain': 9, 
      'Ghost Tools Spirit Gain': 32, 'Ghost Tools Cooldown Reduction': 11,
      'Status Effect Duration': 22, 'Status Effect Damage': 22, 'Execution Damage': 27,
      'Ultimate Gain': 12, 'Ultimate Damage': 21, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    attributes3: ['Expanded Chamber', 'Ignite'],
  },
  {
    id: 'storm_tanzutsu',
    name: 'Storm Tanzutsu',
    category: 'Ghost Tool',
    rarity: 'Legendary',
    icon: '⚡',
    description: 'Massive capacity of 6 bullets that can be fired in quick succession, at the sacrifice of 30% reduced damage',
    stats: { attack: 80, defense: 0, health: 0, resolve: 15, stealth: 0, ranged: 65 },
    perk: 'Each shot releases a lightning arc that hits nearby enemies',
    attributes1: ['Tanzutsu Damage', 'All Ranged Damage', 'Ranged Spirit Gain', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'Tanzutsu Damage': 12, 'All Ranged Damage': 5, 'Ranged Spirit Gain': 12, 
      'Ghost Tools Spirit Gain': 35, 'Ghost Tools Cooldown Reduction': 12,
      'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Execution Damage': 30,
      'Ultimate Gain': 16, 'Ultimate Damage': 25, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    attributes3: ['Expanded Chamber', 'Ignite'],
  },
  {
    id: 'smoke_bomb',
    name: 'Smoke Bomb',
    category: 'Ghost Tool',
    rarity: 'Epic',
    icon: '💣',
    description: 'Engulfs the area around you in smoke, providing temporary invisibility',
    stats: { attack: 10, defense: 15, health: 0, resolve: 10, stealth: 50, ranged: 0 },
    perk: 'Enemies in smoke cannot detect movement',
    attributes1: ['Assassination Damage', 'Assassination Spirit Gain', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'Assassination Damage': 9, 'Assassination Spirit Gain': 11, 
      'Ghost Tools Spirit Gain': 32, 'Ghost Tools Cooldown Reduction': 11,
      'Status Effect Duration': 22, 'Status Effect Damage': 22, 'Execution Damage': 27,
      'Ultimate Gain': 12, 'Ultimate Damage': 21, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    attributes3: ['Armory', 'Poison'],
  },
  {
    id: 'weakening_smoke_bomb',
    name: 'Weakening Smoke Bomb',
    category: 'Ghost Tool',
    rarity: 'Legendary',
    icon: '☁️',
    description: 'Engulfs the area around you in smoke, providing temporary invisibility and Weakening nearby enemies',
    stats: { attack: 15, defense: 20, health: 0, resolve: 15, stealth: 40, ranged: 0 },
    perk: 'Enemies in smoke deal 30% less damage and move 25% slower',
    attributes1: ['Assassination Damage', 'Assassination Spirit Gain', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'Assassination Damage': 10, 'Assassination Spirit Gain': 12, 
      'Ghost Tools Spirit Gain': 35, 'Ghost Tools Cooldown Reduction': 12,
      'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Execution Damage': 30,
      'Ultimate Gain': 16, 'Ultimate Damage': 25, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    attributes3: ['Armory', 'Poison'],
  },
  {
    id: 'caltrops',
    name: 'Caltrops',
    category: 'Ghost Tool',
    rarity: 'Epic',
    icon: '⭐',
    description: 'Creates an area that damages and slows enemies who walk through it',
    stats: { attack: 25, defense: 10, health: 0, resolve: 5, stealth: 15, ranged: 20 },
    perk: 'Enemies slowed by caltrops take increased damage',
    attributes1: ['All Ranged Damage', 'All Ranged Headshot Damage', 'Ranged Spirit Gain', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'All Ranged Damage': 5, 'All Ranged Headshot Damage': 11, 'Ranged Spirit Gain': 9,
      'Ghost Tools Spirit Gain': 32, 'Ghost Tools Cooldown Reduction': 11,
      'Status Effect Duration': 22, 'Status Effect Damage': 22, 'Execution Damage': 27,
      'Ultimate Gain': 12, 'Ultimate Damage': 21, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    attributes3: ['Armory', 'Fire Caltrops', 'Deep Bags'],
  },
  {
    id: 'affliction_caltrops',
    name: 'Affliction Caltrops',
    category: 'Ghost Tool',
    rarity: 'Legendary',
    icon: '☠️',
    description: 'Creates an area that inflicts Poison, Vulnerable, Weaken and Bleed on enemies who walk through it',
    stats: { attack: 35, defense: 5, health: 0, resolve: 10, stealth: 15, ranged: 25 },
    perk: 'Caltrops apply a stacking poison that lingers for 10 seconds',
    attributes1: ['All Ranged Damage', 'All Ranged Headshot Damage', 'Ranged Spirit Gain', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'All Ranged Damage': 5, 'All Ranged Headshot Damage': 12, 'Ranged Spirit Gain': 10,
      'Ghost Tools Spirit Gain': 35, 'Ghost Tools Cooldown Reduction': 12,
      'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Execution Damage': 30,
      'Ultimate Gain': 16, 'Ultimate Damage': 25, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    attributes3: ['Armory', 'Fire Caltrops', 'Deep Bags'],
  },
  {
    id: 'healing_incense',
    name: 'Healing Incense',
    category: 'Ghost Tool',
    rarity: 'Epic',
    icon: '🪔',
    description: 'Deploys incense that heals nearby allies over time',
    stats: { attack: 0, defense: 10, health: 35, resolve: 10, stealth: 5, ranged: 0 },
    perk: 'Releases a small healing cloud on use',
    attributes1: ['All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'All Melee Damage': 5, 'All Melee Stagger Damage': 5, 'Spirit Gain On Melee': 9,
      'Perfect Counter Damage': 30, 'Ghost Tools Spirit Gain': 32, 
      'Ghost Tools Cooldown Reduction': 11,
      'Status Effect Duration': 22, 'Status Effect Damage': 22, 'Execution Damage': 27,
      'Ultimate Gain': 12, 'Ultimate Damage': 21, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 18, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 11,
    },
    attributes3: ['Armory', 'Larger Heal Area'],
  },
  {
    id: 'purified_healing_incense',
    name: 'Purified Healing Incense',
    category: 'Ghost Tool',
    rarity: 'Legendary',
    icon: '🕯️',
    description: 'Deploys incense that grants Spirit and heals nearby allies over time',
    stats: { attack: 0, defense: 20, health: 50, resolve: 20, stealth: 10, ranged: 0 },
    perk: 'Releases a healing aura that restores health and removes status effects',
    attributes1: ['All Melee Damage', 'All Melee Stagger Damage', 'Spirit Gain On Melee', 'Perfect Counter Damage', 'Ghost Tools Spirit Gain', 'Ghost Tools Cooldown Reduction'],
    attributes2: ['Status Effect Duration', 'Status Effect Damage', 'Execution Damage', 'Ultimate Gain', 'Ultimate Damage', 'Spirit Move Damage', 'Thrown Weapon Damage', 'Perfect Parry Window', 'Class Ability Cooldown Reduction'],
    attributeMaxValues: {
      'All Melee Damage': 5, 'All Melee Stagger Damage': 5, 'Spirit Gain On Melee': 10,
      'Perfect Counter Damage': 35, 'Ghost Tools Spirit Gain': 35, 
      'Ghost Tools Cooldown Reduction': 12,
      'Status Effect Duration': 25, 'Status Effect Damage': 25, 'Execution Damage': 30,
      'Ultimate Gain': 16, 'Ultimate Damage': 25, 'Spirit Move Damage': 20,
      'Thrown Weapon Damage': 20, 'Perfect Parry Window': 36, 'Class Ability Cooldown Reduction': 12,
    },
    attributes3: ['Armory', 'Larger Heal Area'],
  },
];

export const getGearsByCategory = (
  category: GearCategory,
  meleeWeaponType?: MeleeWeaponType,
  rangedWeaponTypes?: RangedWeaponType[],
  allowedItemIds?: string[],
): Gear[] =>
  GEARS.filter((g) => {
    if (g.category !== category) return false;
    if (allowedItemIds && allowedItemIds.length > 0) {
      return allowedItemIds.includes(g.id);
    }
    if (category === 'Melee' && meleeWeaponType) {
      return g.weaponType === meleeWeaponType;
    }
    if (category === 'Range' && rangedWeaponTypes && rangedWeaponTypes.length > 0) {
      return g.rangedWeaponType !== undefined && rangedWeaponTypes.includes(g.rangedWeaponType);
    }
    return true;
  });

export const getGearById = (id: string | undefined): Gear | undefined =>
  GEARS.find((g) => g.id === id);

export const RARITY_COLOR: Record<GearRarity, string> = {
  Common:    '#9ca3af',
  Rare:      '#3b82f6',
  Epic:      '#c084fc',
  Legendary: '#f59e0b',
};

// Re-export for convenience
export type { GearCategory, GearRarity, StatKey };
